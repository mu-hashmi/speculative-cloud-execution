...
class ImageServer(image_pb2_grpc.GRPCImageServicer):
    def SimpleMethod(self, request, context):
        ...
        response = image_pb2.Response(ack_data=RESPONSE, req_id=request.req_id, recv_time=recv_time)
        return response
        
    def BidirectionalStreamingMethod(self, request_iterator, context):  
        for request in request_iterator:
            recv_time = time.time()
            print("recv from client message size %d id %d" % (len(request.image_data), request.req_id))
            # image_received = process_image(request.image_data)
            image_received = process_dummy_image(request.image_data)
            #print(image_received)
            yield image_pb2.Response(ack_data=RESPONSE, req_id=request.req_id, recv_time=recv_time)
